...
class VoiceClient:
    ...
    def __init__(self, audio_handler: Optional[Callable] = None):
        ...
        self.headers = {"Authorization": f"Bearer {self.config.OPENAI_API_KEY}", "OpenAI-Beta": "realtime=v1"}

    async def connect(self):
        """Establish WebSocket connection"""
        try:
            self.logger.info("Connecting to OpenAI Realtime API...")
            self.websocket = await websockets.connect(self.ws_url, extra_headers=self.headers)
