...
async def run_audio_file_test(audio_file: str):
    ...
    try:
        ...
        scenarios = [{"description": f"Test with audio file: {audio_file}", "audio_file": audio_file}]
...
Examples:
  ...
  python -m python_demo_voice_app.cli config                 # Show configuration
    ...
    parser.add_argument("command", choices=["run", "test", "test-audio", "config"], help="Command to execute")

    parser.add_argument("audio_file", nargs="?", help="Audio file path (required for test-audio command)")
